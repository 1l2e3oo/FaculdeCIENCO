...
create trigger trg_insertAula before insert on Aula for each row
set new.dataInclusao = now();

alter table Aula add column atualizacao date;

create trigger tgr_atualizacaoAula before update on Aula for each row
set new.atualizacao = now();

update Aula set dataInclusao = '2022-0-14' where idAula = 1;
...
create trigger trg_comicao before insert on Servicos for each row
set new.comicao = (new.valor*0.1);

select*from Servicos;

insert into Servicos(descricao, valor) values('Pintura', 2000.00);
